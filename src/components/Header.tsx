...
export default function Header() {
  const { title, subPeriod, tag, contestant } = CONTEST_INFO;
  ...
  return (
    <header className="relative bg-gradient-to-br from-[#7f0000] via-[#991b1b] to-[#b71c1c] text-white border-b-4 border-amber-400 shadow-md">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14 text-center relative z-10">
        
        {/* Emblem & Top Tag */}
        <div className="inline-flex items-center gap-2 bg-amber-400 text-red-950 px-4 py-1.5 rounded-full font-heading text-xs sm:text-sm font-bold tracking-wider uppercase shadow-xs mb-4">
          <Award className="w-4 h-4 text-red-900" />
          <span>{tag}</span>
        </div>

        {/* Main Title */}
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold uppercase font-heading tracking-tight leading-tight max-w-4xl mx-auto drop-shadow-md text-amber-50">
          <span className="block">{CONTEST_INFO.titleLine1}</span>
          <span className="block mt-1 sm:mt-1.5">{CONTEST_INFO.titleLine2}</span>
        </h1>

        {/* Subtitle / Timeline */}
        <div className="mt-3 text-lg sm:text-xl font-semibold text-amber-200 font-heading tracking-wide">
          {subPeriod}
        </div>
